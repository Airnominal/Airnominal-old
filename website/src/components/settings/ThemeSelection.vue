  ...
  <v-card width="35rem">
    ...
    <v-card-text class="text--primary mb-n12">
      <v-radio-group v-model="themeSelection">
        <v-radio :key="0" :value="0" class="pb-2" label="System" />
        <v-radio :key="1" :value="1" class="pb-2" label="Light" />
        <v-radio :key="2" :value="2" label="Dark" />
      </v-radio-group>
    </v-card-text>

    <v-card-actions class="justify-end">
      <v-btn text v-on:click=closeDialog>Ok</v-btn>
    </v-card-actions>
  </v-card>
</template>
